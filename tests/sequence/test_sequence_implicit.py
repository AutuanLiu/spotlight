...
def test_implicit_cnn_synthetic(randomness, expected_mrr):
    ...
    model = ImplicitSequenceModel(
        loss=LOSS,
        representation=CNNNet(train.num_items, embedding_dim=EMBEDDING_DIM, kernel_width=5, num_layers=1),
        batch_size=BATCH_SIZE,
        learning_rate=1e-2,
        l2=0.0,
        n_iter=NUM_EPOCHS * 5,
...
def test_implicit_cnn_dilation_synthetic(num_layers, dilation, expected_mrr):
    ...
    random_state = np.random.RandomState(RANDOM_SEED)
    train, test = _get_synthetic_data(randomness=1e-03, num_interactions=20000, random_state=random_state)

    model = ImplicitSequenceModel(
        loss=LOSS,
        representation=CNNNet(
            train.num_items, embedding_dim=EMBEDDING_DIM, kernel_width=3, dilation=dilation, num_layers=num_layers),
        batch_size=BATCH_SIZE,
        learning_rate=1e-2,
        l2=0.0,
...
def test_implicit_pooling_losses(loss, expected_mrr):
    ...
    random_state = np.random.RandomState(RANDOM_SEED)
    train, test = _get_synthetic_data(randomness=1e-3, random_state=random_state)
...
def test_bloom_cnn(compression_ratio, expected_mrr):
    ...
    representation = CNNNet(
        train.num_items, embedding_dim=EMBEDDING_DIM, kernel_width=3, item_embedding_layer=embedding)
