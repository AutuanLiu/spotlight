...
def test_explicit_serialization(data):
    ...
    model = ExplicitFactorizationModel(
        loss='regression', n_iter=3, batch_size=1024, learning_rate=1e-3, l2=1e-5, use_cuda=CUDA)
    model.fit(train)
...
def test_implicit_serialization(data):
    ...
    model = ImplicitFactorizationModel(
        loss='bpr', n_iter=3, batch_size=1024, learning_rate=1e-2, l2=1e-6, use_cuda=CUDA)
    model.fit(train)
...
def test_implicit_sequence_serialization(data):
    ...
    model = ImplicitSequenceModel(
        loss='bpr',
        representation=CNNNet(train.num_items, embedding_dim=32, kernel_width=3, dilation=(1, ), num_layers=1),
        batch_size=128,
        learning_rate=1e-1,
        l2=0.0,
        n_iter=5,
