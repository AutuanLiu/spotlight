...
@pytest.mark.parametrize('embedding_class', [nn.Embedding, ScaledEmbedding, BloomEmbedding])
def test_embeddings(embedding_class):
    ...
    layer = embedding_class(num_embeddings, embedding_dim)

    # Test 1-d inputs (minibatch)
    indices = torch.from_numpy(np.random.randint(0, num_embeddings, size=batch_size, dtype=np.int64))
    ...
    indices = torch.from_numpy(np.random.randint(0, num_embeddings, size=(batch_size, sequence_length), dtype=np.int64))
    representation = layer(indices)
