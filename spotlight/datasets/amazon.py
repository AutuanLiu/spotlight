...
def _download_amazon():
    ...
    extension = '.hdf5'
    url = ('https://github.com/maciejkula/recommender_datasets/' 'releases/download')
    version = '0.1.0'

    path = _transport.get_data('/'.join((url, version, 'amazon_co_purchasing' + extension)), 'amazon',
                               'amazon_co_purchasing{}'.format(extension))

    with h5py.File(path, 'r') as data:
        return (data['/user_id'][:], data['/item_id'][:], data['/rating'][:], data['/timestamp'][:],
                data['/features_item_id'][:], data['/features_feature_id'][:])
...
def get_amazon_dataset(min_user_interactions=10, min_item_interactions=10):
    ...
    (user_ids, item_ids, ratings, timestamps, feature_item_ids, feature_ids) = _download_amazon()
    ...
    retain = np.logical_and(np.in1d(user_ids, retain_user_ids), np.in1d(item_ids, retain_item_ids))
