...
def _generate_sequences(user_ids, sequence_elements, indices, max_sequence_length, step_size):
    ...
    for i in range(len(indices)):
        ...
        for seq in _sliding_window(sequence_elements[start_idx:stop_idx], max_sequence_length, step_size):
...
class Interactions(object):
    ...
    def __init__(self, user_ids, item_ids, ratings=None, timestamps=None, weights=None, num_users=None, num_items=None):
    ...
    def __repr__(self):
        return ('<Interactions dataset ({num_users} users x {num_items} items '
                'x {num_interactions} interactions)>'.format(
                    num_users=self.num_users, num_items=self.num_items, num_interactions=len(self)))
    ...
    def _check(self):
        ...
        for name, value in (('item IDs', self.item_ids), ('ratings', self.ratings), ('timestamps', self.timestamps),
    ...
    def to_sequence(self, max_sequence_length=10, min_sequence_length=None, step_size=None):
        ...
        if self.timestamps is None:
            raise ValueError('Cannot convert to sequences, ' 'timestamps not available.')

        if 0 in self.item_ids:
            raise ValueError('0 is used as an item id, conflicting ' 'with the sequence padding value.')
        ...
        if weighted:
            weight_sequences = np.zeros((num_subsequences, max_sequence_length), dtype=np.int32)
        sequence_users = np.empty(num_subsequences, dtype=np.int32)
        for i, (uid, seq) in enumerate(
                _generate_sequences(user_ids, item_ids, indices, max_sequence_length, step_size)):
            sequences[i][-len(seq):] = seq
            sequence_users[i] = uid

        if weighted:
            for i, (uid, seq) in enumerate(
                    _generate_sequences(user_ids, weights, indices, max_sequence_length, step_size)):
        ...
        return (SequenceInteractions(
            sequences, user_ids=sequence_users, weight_sequences=weight_sequences, num_items=self.num_items))
...
class SequenceInteractions(object):
    ...
    def __init__(self, sequences, user_ids=None, weight_sequences=None, num_items=None):
    ...
    def __repr__(self):
        ...
        return ('<Sequence interactions dataset ({num_sequences} '
                'sequences x {sequence_length} sequence length)>'.format(
