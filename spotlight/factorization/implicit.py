...
class ImplicitFactorizationModel(object):
    ...
    def fit(self, interactions, verbose=False):
        ...
        for epoch_num in range(self._n_iter):
            users, items, sample_weights = shuffle(user_ids, item_ids, sample_weights, random_state=self._random_state)
            ...
            if sample_weights is not None:
                sample_weights_tensor = gpu(torch.from_numpy(sample_weights), self._use_cuda)

            epoch_loss = 0.0
            for (minibatch_num, (batch_user, batch_item, batch_sample_weights)) in enumerate(
                    minibatch(user_ids_tensor, item_ids_tensor, sample_weights_tensor, batch_size=self._batch_size)):
                ...
                loss = self._loss_func(positive_prediction, negative_prediction, sample_weights=batch_sample_weights)
