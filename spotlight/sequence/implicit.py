...
from spotlight.sequence.representations import (PADDING_IDX, CNNNet, LSTMNet, MixtureLSTMNet, PoolNet)
...
class ImplicitSequenceModel(object):
    ...
    def __init__(self,
        ...
        if isinstance(representation, str):
            assert representation in ('pooling', 'cnn', 'lstm', 'mixture')
    ...
    def _get_multiple_negative_predictions(self, shape, user_representation, n=5):
        batch_size, sliding_window = shape
        size = (n, ) + (1, ) * (user_representation.dim() - 1)
        negative_prediction = self._get_negative_prediction((n * batch_size, sliding_window),
